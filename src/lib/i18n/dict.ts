...
export const DICT = {
  ...
  "proveEdge.title": {
    en: "How much volume makes the result convincing",
    ru: "Сколько нужно сыграть, чтобы проверить плюс",
  },
  "proveEdge.question": {
  ...
  "proveEdge.intro": {
    en: "This block estimates the sample size needed before a positive ROI stops looking like random tournament noise. It is not a profit forecast: it answers how much volume is needed to trust the signal. The smaller the expected ROI, the faster the required distance grows.",
    ru: "Этот блок оценивает, сколько турниров нужно, чтобы положительный ROI перестал выглядеть как случайный разброс. Это не прогноз прибыли, а ответ на вопрос: какой объем нужен, чтобы доверять сигналу. Чем меньше ожидаемый плюс, тем быстрее растет нужная дистанция.",
  },
  ...
  "proveEdge.anchor.body": {
    ...
    ru: "если реальный плюс равен {roi}, нужно примерно {tourneys} турниров при уверенности {ci}%. Это около {fields} полных полей по {afs} игроков.",
  },
  "proveEdge.col.roi": {
    en: "If the real ROI is",
    ru: "Если реальный плюс",
  },
  "proveEdge.col.sigma": { en: "Result noise", ru: "Шум результата" },
  "proveEdge.col.tourneys": { en: "Needed tournaments", ru: "Нужно турниров" },
