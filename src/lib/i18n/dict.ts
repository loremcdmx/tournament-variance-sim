...
export const DICT = {
  ...
  "chart.convergence.mode.hint": {
    en: "Averaged: generic planning mode. Freeze and Battle Royale use runtime single-row compiles at the chosen controls, while PKO and Mystery use promoted format fits. Mix weights are shares of tournaments in the synthetic mix. Schedule: compiles the real rows and aggregates per-row dollar variance, field variability, payout shape, rake, and bounty structure into one schedule-aware σ_ROI.",
    ru: "Усреднённо: общий planning-режим. Для Фриза и Battle Royale берётся runtime single-row компиляция на выбранных контролах, а для ПКО и Мистери используются промоутнутые format-fits. В Миксе веса означают долю турниров в синтетическом миксе. Расписание: компилирует реальные строки и агрегирует долларовую дисперсию по строкам, field variability, payout-shape, рейк и bounty-структуру в один schedule-aware σ_ROI.",
  },
