...
export type RowFormat = "freeze" | "pko" | "mystery" | "mystery-royale";
...
export const SIGMA_ROI_FREEZE: SigmaRoiFit = {
  C0: 0.6564,
  C1: 0,
  beta: 0.3694,
  resid: 0.06,
...
export const SIGMA_ROI_PKO: SigmaRoiFit = {
  C0: 0.6265,
  C1: 0.4961,
  beta: 0.2763,
  resid: 0.08,
...
export const SIGMA_ROI_MYSTERY: SigmaRoiFit = {
  C0: 2.5164,
  C1: 3.7097,
  beta: 0.1325,
  resid: 0.18,
...
export const SIGMA_ROI_MYSTERY_ROYALE: SigmaRoiFit = {
  C0: 8.1534,
  C1: 7.9063,
  beta: 0,
  resid: 0.02,
...
export const SIGMA_COEF_BY_FORMAT: Record<RowFormat, SigmaRoiFit> = {
...
export const FIT_RAKE_BY_FORMAT: Record<RowFormat, number> = {
  freeze: 0.10,
  pko: 0.10,
  mystery: 0.10,
  "mystery-royale": 0.08,
...
export function inferRowFormat(row: TournamentRow): RowFormat {
...
export function sigmaRoiForRow(
  ...
  const sigma = sigmaForFit(coef, afs, roi, rakeScale);
  return {
    sigma,
...
export function effectiveConvergenceFormats(
  format: ConvergenceFormat,
  mode: "avg" | "exact",
  mix: MixTuple,
  schedule?: readonly TournamentRow[] | null,
): RowFormat[] {
  if (mode === "exact" && schedule) return schedule.map(inferRowFormat);
  if (
    format === "freeze" ||
    format === "pko" ||
    format === "mystery" ||
