...
- **KO-доля EV** — слайдер "KOs as % of gross EV" двигает долю
  профита из ноков. В Battle Royale средний размер конверта фиксирован
  таблицей, поэтому меняется ожидаемое число ноков и форма finish-PMF;
  fixed-ITM ряды закрывают KO-бюджет остатком после cash EV, чтобы общий ROI
  не ехал.
...
│   │   ├── gameType.ts       # freezeout / pko / mystery пресеты полей
...
│   │   ├── progress*.ts      # pure progress math / constants shared with UI
│   │   └── *.test.ts         # vitest — 24 файла, 400+ тестов, запускаются через `npm test`
│   │
│   ├── i18n/
│   │   ├── dict.ts           # плоский словарь en/ru (все UI строки тут)
│   │   └── LocaleProvider.tsx
│   ├── results/             # post-hoc result helpers (RB, jackpots, satellites, ref lines)
│   ├── ui/
│   │   ├── AdvancedModeProvider.tsx
│   │   ├── etaEstimator.ts   # pure ETA smoothing / projection math
│   │   ├── progressBarState.ts # pure run-bar state machine
...
- **Seeding per-sample** через `mixSeed(seed, sampleIdx)` (отдельные XOR-смещённые потоки на каждый стохастический канал). sampleIdx — глобальный (0..samples), а не per-shard. Это даёт одинаковые дравы независимо от shard-разбиения.
...
- [`docs/FITTING.md`](docs/FITTING.md) — fitting workflow, drift-report и какие коэффициенты реально промотируются в UI
- [`docs/REPO_ATLAS.md`](docs/REPO_ATLAS.md) — короткая карта репо: где engine, где UI, где hotspots и откуда начинать изменения
- [`CONTRIBUTING.md`](CONTRIBUTING.md) — dev-workflow, тесты, стиль коммитов, PR
- [`scripts/README.md`](scripts/README.md) — какие скрипты каноничны, а какие диагностические или исследовательские
...
- **KO EV share control** reports how much gross EV comes from knockouts. In
  ...
  as residual after cash EV so total ROI remains pinned.
...
Next.js 16, React 19, TypeScript, Web Worker engine pool, uPlot, Tailwind 4, Vitest (400+ tests across engine invariants and UI math).
